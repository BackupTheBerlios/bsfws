...
For every script you want to use, create a \ttfamily{<script>} tag. This script tag must have at least two attributes:

\begin{enumerate}

\item The \ttfamily{type} attribute that specifies that this script is a script that shall be interpreted using BWS and the programming language the script is written in; this attribute is given as \texttt{bws/scriptengine}, for example it is \ttfamily{bws/netrexx} for the NetRexx interpreter or \ttfamily{bws/javascript} for the Rhino JavaScript interpreter.

\item The \ttfamily{id} attribute unter which this script is referenced in script calls. This id, as well as any other id used in the document, must be unique, it may consist of only alphanumeric characters (A-Z, a-z, 0-9), dashes (-), underscores (\_), dots (.) and colons (:).
